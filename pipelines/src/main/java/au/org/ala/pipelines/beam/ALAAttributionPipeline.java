...
public class ALAAttributionPipeline {
  ...
  public static void run(InterpretationPipelineOptions options) {
    ...
    UnaryOperator<String> pathMetadataFn = t -> FsUtils.buildPathInterpretUsingTargetPath(options, t, "*" + AVRO_EXTENSION);

    UnaryOperator<String> pathFn = t -> FsUtils.buildPathInterpretUsingTargetPath(options, t, id);

    log.info("Creating transformations for attributionpipeline");
...
//    ALACollectionTransform alaCollectionTransform = ALACollectionTransform.create(properties);
...
//    p.apply("Read Metadata", metadataTransform.read(pathMetadataFn))
//            .apply("Interpret Sampling", alaAttributionTransform.interpret());
////
//    p.apply("Read ExtendedRecords", verbatimTransform.read(options.getInputPath()))
//            .apply("Read occurrences from extension", alaCollectionTransform.interpret())
//            .apply("Write sampling to avro", alaAttributionTransform.write(pathFn));
