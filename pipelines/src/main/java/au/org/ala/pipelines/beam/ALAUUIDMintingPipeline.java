 ...
 * that have been previously loaded. <p> This works by: <p> 1. Creating a map from ExtendedRecord of
 * UniqueKey -> ExtendedRecord.getId(). The UniqueKey is constructed using the unique terms
 * specified in the collectory. <p> 2. Creating a map from source of UniqueKey -> UUID from previous
 * ingestions (this will be blank for newly imported datasets) <p> 3. Joining the two maps by the
 * UniqueKey. <p> 4. Writing out ALAUUIDRecords containing ExtendedRecord.getId() -> (UUID,
 * UniqueKey). These records are then used in SOLR index generation. <p> 5. Backing up previous AVRO
 * ALAUUIDRecords. <p> The end result is a AVRO export ALAUUIDRecords which are then used as a
 * mandatory extension in the generation of the SOLR index.
 *
 * @see ALAUUIDRecord
 */
@Slf4j
...
public class ALAUUIDMintingPipeline {
  ...
  public static void run(InterpretationPipelineOptions options) throws Exception {
    ...
    String alaRecordDirectoryPath =
        options.getTargetPath() + "/" + options.getDatasetId().trim() + "/1/identifiers/"
    ...
    KeyValueStore<String, ALACollectoryMetadata> dataResourceKvStore = ALAAttributionKVStoreFactory
    ...
    final List<String> uniqueTerms = collectoryMetadata.getConnectionParameters()
        .getTermsForUniqueKey();
    ...
    for (String uniqueTerm : uniqueTerms) {
      ...
      if (dwcTerm.isPresent()) {
        ...
        throw new RuntimeException(
            "Unrecognised unique term configured for datasource " + options.getDatasetId()
                + ", term: " + uniqueTerm);
      }
    }
    ...
    PCollection<KV<String, String>> extendedRecords =
        p.apply(AvroIO.read(ExtendedRecord.class).from(
            options.getTargetPath() + "/" + options.getDatasetId().trim()
                + "/1/interpreted/verbatim/*.avro"))
            .apply(ParDo.of(new DoFn<ExtendedRecord, KV<String, String>>() {
              @ProcessElement
              public void processElement(@Element ExtendedRecord source,
                  OutputReceiver<KV<String, String>> out, ProcessContext c) {
    ...
    PCollection<KV<String, KV<String, String>>> joinedPcollection =
        org.apache.beam.sdk.extensions.joinlibrary.Join
            .fullOuterJoin(extendedRecords, alaUuids, NO_ID_MARKER, NO_ID_MARKER);
    ...
    if (new File(alaRecordDirectoryPath).exists()) {
      FileUtils.rename(new File(alaRecordDirectoryPath),
          new File(alaRecordDirectory + "_backup_" + System.currentTimeMillis()));
    }
   ...
   */
  public static String generateUniqueKey(ExtendedRecord source, List<DwcTerm> uniqueDwcTerms)
    ...
    for (DwcTerm dwcTerm : uniqueDwcTerms) {
      ...
      if (value == null || StringUtils.trimToNull(value) == null) {
        throw new RuntimeException(
            "Unable to load dataset. Unique term empty for record term: " + dwcTerm);
      }
  ...
  static class CreateALAUUIDRecordFcn extends DoFn<KV<String, KV<String, String>>, ALAUUIDRecord> {
    ...
    private Counter orphanedUniqueKeys = Metrics
        .counter(CreateALAUUIDRecordFcn.class, "orphanedUniqueKeys");
    private Counter newUuids = Metrics.counter(CreateALAUUIDRecordFcn.class, "newUuids");
    private Counter preservedUuids = Metrics
        .counter(CreateALAUUIDRecordFcn.class, "preservedUuids");

    @ProcessElement
    public void processElement(@Element KV<String, KV<String, String>> uniqueKeyMap,
      ...
      ALAUUIDRecord aur = ALAUUIDRecord.newBuilder().setUniqueKey(uniqueKey).setUuid(uuid).setId(id)
          .build();
      out.output(aur);
    }
  }
  ...
  static class ALAUUIDRecordKVFcn extends DoFn<ALAUUIDRecord, KV<String, String>> {
    ...
    @ProcessElement
    public void processElement(@Element ALAUUIDRecord alaUUIDRecord,
