...
public class ALASamplingToAvroPipeline {
  ...
  public static void run(InterpretationPipelineOptions options) {
    ...
    String alaRecordDirectoryPath = options.getTargetPath() + "/" + options.getDatasetId().trim()
        + "/1/interpreted/" + PipelinesVariables.Pipeline.Interpretation.RecordType.LOCATION.name()
        .toLowerCase();

    // Filter records without lat/long, create map LatLng -> ExtendedRecord.getId()
    PCollection<KV<String, String>> latLngID = p
        ...
        .apply(ParDo.of(new DoFn<LocationRecord, LocationRecord>() {
          @ProcessElement
          public void processElement(@Element LocationRecord locationRecord,
    ...
    PCollection<KV<String, Map<String, String>>> alaSampling = lines
        .apply(ParDo.of(new DoFn<String, KV<String, Map<String, String>>>() {
          @ProcessElement
          public void processElement(@Element String sampling,
                ...
                new DoFn<KV<String, CoGbkResult>, AustraliaSpatialRecord>() {
                  ...
                  public void processElement(ProcessContext c) {
                    ...
                    while (idIter.hasNext()) {
                      ...
                      AustraliaSpatialRecord aur = AustraliaSpatialRecord.newBuilder()
                          .setItems(sampling).setId(id).build();
                      c.output(aur);
                    }
                  }
                }));
    ...
    australiaSpatialRecordPCollection
        .apply("Write sampling to avro", australiaSpatialTransform.write(outputPath));
  ...
  private static String[] getColumnHeaders(String samplingPath) {
    ...
    if (samplingDir.exists() && samplingDir.isDirectory()) {
      ...
      if (samplingFiles.length > 0) {
        ...
        try {
          String columnHeaderString = new BufferedReader(new FileReader(samplingFiles[0]))
              .readLine();
  ...
  static class LocationRecordFcn extends DoFn<LocationRecord, KV<String, String>> {
    ...
    public void processElement(@Element LocationRecord locationRecord,
      ...
      try {
        KV<String, String> kv = KV
            .of(locationRecord.getDecimalLatitude() + "," + locationRecord.getDecimalLongitude(),
