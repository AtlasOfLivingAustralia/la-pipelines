 ...
 * output directory. <p> TODO This needs improvement to make it a more useable commandline tool with
 * options.
 */
@Slf4j
    ...
    if (args.length == 1) {
      ...
      if (!latLngExportDir.exists()) {
        throw new RuntimeException(
            "LatLng export unavailable. Has LatLng export pipeline been ran ? " + latLngExportDir
                .getAbsolutePath());
      }
      ...
      log.info("Finished sampling for {}. Time taken {} minutes", dataSetID,
...
//                .filter(s -> s.startsWith("cl") || requiredEls.contains(s))
  ...
  public void crawl(String layers, File inputFile, File outputDirectory) throws Exception {
    ...
    for (List<String> partition : partitioned) {
      ...
      Response<SamplingService.Batch> submit = service.submitIntersectBatch(layers, coords)
          .execute();
      ...
      while (!state.equalsIgnoreCase(FINISHED_STATUS) && !state.equalsIgnoreCase(ERROR_STATUS)) {
        ...
        log.info("batch ID {} - status: {} - time elapses {} seconds", batchId, state,
        ...
        if (!state.equals(FINISHED_STATUS)) {
          ...
          try (ZipInputStream zipInputStream = new ZipInputStream(
              new FileInputStream(new File(outputDirectory, batchId + ".zip")))) {
   ...
   */
  private boolean downloadFile(File outputDirectory, String batchId,
      SamplingService.BatchStatus batchStatus) throws IOException {
      ...
      try {
        try (
            ReadableByteChannel readableByteChannel = Channels
                .newChannel(new URL(batchStatus.getDownloadUrl()).openStream());
            FileOutputStream fileOutputStream = new FileOutputStream(
                new File(outputDirectory, batchId + ".zip"));
  ...
  /**
   * Util to partition a stream into fixed size windows. See https://e.printstacktrace.blog/divide-a-list-to-lists-of-n-size-in-Java-8/
   */
  ...
  public static void unzipFiles(final ZipInputStream zipInputStream,
      final java.nio.file.Path unzipFilePath) throws IOException {
